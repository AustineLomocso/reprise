...
## ADR-13 Sample data is labelled as sample data

Context: The first build slice (web-only, see `../PROGRESS.md`) deploys the dashboard before any live triage has run, using generated sample records. A visitor or judge must never mistake illustrative records for results of a live run, and the same dashboard must serve live records later without code changes.
Decision: The site index gains one field, `data_source: "sample" | "live"`. `reprise build-site` takes `--data-source sample|live` (default `live`) and writes it into `data/index.json`. When it is `sample`, every route shows a persistent banner: "Sample data. These reports illustrate how Reprise works; they were not produced by a live run." In sample mode, references to GitHub issues, pull requests, commits and branches are shown as plain text instead of links, because the sample objects do not exist on GitHub. Sample records are produced by `scripts/generate-sample-records.mjs`, which computes every statistic with the engine's `src/stats` functions, and are committed to `test/fixtures/sample-records/`.
Consequences: This is the only contract amendment in the web-only slice. Live deployments (`reprise-pages.yml` reading `reprise-data`) omit the flag and get `live`. The banner cannot be removed by data: it is driven only by the build flag.
